...
namespace LINQ_Examples
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            foreach (var c in cusOrderByFirstname)
            {
                Console.WriteLine("{0}", c.First);
            }

            Console.WriteLine("\nExercise 4b:");
            var cusOrderBylastname =
                from c in customers
                orderby c.Last.Length
                select c;

            foreach (var c in cusOrderBylastname)
            {
            ...
            foreach (var c in cusOrderByPrice)
            {
                Console.WriteLine("{0}", c.Price);
            }

            Console.WriteLine("\nExercise 4d:");
            var cusOrderByFirstThen =
                from c in customers
                orderby c.First.Length, c.Last
                select c;

            foreach (var c in cusOrderByFirstThen)
            {
                Console.WriteLine("{0} {1}", c.First, c.Last);
            }
            ...
            var cusGroupFirst =
                from c in customers
                orderby c.First 
                group c by c.First[0] into letter
                select new { FirstLetter = letter.Key, Name = letter };

            foreach (var letter in cusGroupFirst)
            {
                Console.WriteLine("{0}:", letter.FirstLetter);
                foreach (var n in letter.Name)
                {
            ...
            var cusGroupState =
                from c in customers
                orderby c.State
                group c by c.State into s
                select new { FirstLetter = s.Key, cust = s };

            foreach (var letter in cusGroupState)
            {
                Console.WriteLine("{0}:", letter.FirstLetter);
                foreach (var n in letter.cust)
                {
                    Console.WriteLine(n.First);
                }
                Console.WriteLine();
            ...
            var custNotEqualFirst = custFirst.Except(custLast);

            foreach (var output in custNotEqualFirst)
            {
            ...
            bool accepted = customers.Any(c => c.First.Contains("ed"));
            if(accepted)
                Console.WriteLine("Yes list contains ed");
            else
                Console.WriteLine("no list dosn't contains ed");
